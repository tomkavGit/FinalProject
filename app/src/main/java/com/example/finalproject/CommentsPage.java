...
public class CommentsPage extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        topicsList.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id)
            {
                ...
                new CommentPaginationHandler(CommentsPage.this).execute(position);
        ...
        navView.setNavigationItemSelectedListener(new NavigationView.OnNavigationItemSelectedListener()
        {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem menuItem)
            {
                ...
                if(id == R.id.messages)
                {
                    Intent commentPage = new Intent(this, )
                    //do something
                }
        ...
        nextButton.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v)
            {
                ...
                new TopicPaginationHandler(CommentsPage.this).execute("Next");
            }
        });
        ...
        previousButton.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v)
            {
                ...
                new TopicPaginationHandler(CommentsPage.this).execute("Prev");
            }
        });
        ...
        sortingButton.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v)
            {
                ...
                new TopicPaginationHandler(CommentsPage.this).execute("refresh");
            }
        });
    ...
    private static class TopicPaginationHandler extends AsyncTask<String, Void, Void>
    {
        ...
        int newTopicPage;
        private WeakReference<CommentsPage> activityReference;

        TopicPaginationHandler(CommentsPage context)
        {
        ...
        protected void onPreExecute()
        {
            ...
            CommentsPage appHome = activityReference.get();

            if(appHome == null || appHome.isFinishing())
            {
                return;
            }

            //has access to UI thread for the button reference
            Button sortingButton = appHome.findViewById(R.id.sortingButton);
        ...
        protected Void doInBackground(String ...buttonText)
        {
            ...
            newTopicPage = appHome.topicPage;
            ...
            else if(buttonPagination.equals("Next") && appHome.hasNextPage)
            {
                newTopicPage ++;
            }
            else
            ...
            try
            {
                ...
                try
                {
                    ...
                    response = appHome.convertStreamToString(input);
                ...
                try
                {
                    appHome.topics = gson.fromJson(response, new TypeToken<ArrayList<Topic>>(){}.getType());
                }
                catch(Exception e)
                {
        ...
        protected void onPostExecute(Void voided)
        {
            ...
            String comments;
            String dateTime;
            System.out.println("DEBUG Topics: " + appHome.topics);
            //set the button to the updated text
            if(appHome.topics != null)
            {
                System.out.println("DEBUG: Clearing list then populating it");
                appHome.arrayAdapter.clear();

                for(int count = 0; count < appHome.topics.size(); count++)
                {
                    if(count >= 10)
                    {
                        appHome.hasNextPage = true;
                        //dont allow count to go over 10
                        break;
                    }

                    topicTitle = appHome.topics.get(count).getTopicTitle();
                    username = appHome.topics.get(count).getUsername();
                    comments = "" + appHome.topics.get(count).getComments();
                    dateTime = appHome.topics.get(count).getDateTime();

                    appHome.arrayAdapter.add(topicTitle +" User: " + username +
                    " Comments: " + comments+ " Posted: " + dateTime);

                    //if there are less than 11 results there isn't a new page
                    appHome.hasNextPage = false;
                }

                //updating the global variable to reflect the current state of the app
                //updates the page number at the bottom too
                appHome.topicPage = newTopicPage;
                TextView pageNo = appHome.findViewById(R.id.pageNo);
    ...
    private static class CommentPaginationHandler extends AsyncTask<Integer, Void, ArrayList<Topic>>
    {
        ...
        CommentPaginationHandler(CommentsPage context)
        {
            activityReference = new WeakReference<>(context);
        }
